...
export async function login(data: LoginRequest): Promise<LoginResponse> {
  ...
  const user: User = {
    ...
    enterpriseId: backendUser.enterprise_id as number | null | undefined,
...
export const getCurrentUser = (): Promise<User> =>
  http.get<{ code: number; message: string; data: Record<string, unknown> | null }>('/v1/auth/me').then((res) => {
    ...
    return {
      ...
      enterpriseId: u.enterprise_id as number | null | undefined,
      userType: u.user_type as User['userType'],
