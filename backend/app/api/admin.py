...
from app.models import User, Enterprise, Message, ContentRecord
...
async def list_enterprises(
    ...
    try:
        ...
        if search:
            query = query.filter(
                (Enterprise.name.like(f"%{search}%")) |
                (Enterprise.license_no.like(f"%{search}%"))
            )
        ...
        return APIResponse(
            ...
            message="success",
            data={
                "items": [e.to_dict() for e in enterprises],
                "total": total,
                "page": page,
                "page_size": page_size
            }
...
async def get_stats(
    ...
    try:
        ...
        # 企业统计
        total_enterprises = db.query(func.count(Enterprise.id)).filter(
            Enterprise.deleted_at.is_(None)
        ).scalar()
        verified_enterprises = db.query(func.count(Enterprise.id)).filter(
            Enterprise.deleted_at.is_(None),
            Enterprise.verify_status == "verified"
        ).scalar()
...
async def get_ai_usage(
    ...
    try:
        ...
        ).filter(ContentRecord.created_at >= start_date).first()
