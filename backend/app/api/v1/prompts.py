...
router = APIRouter(tags=["Prompt Management"], responses={404: {"description": "Not found"}})
...
async def list_templates(manager: PromptTemplateManager = Depends(get_template_manager)):
    """获取所有可用的Prompt模板"""
    try:
...
async def render_prompt(request: PromptRenderRequest, manager: PromptTemplateManager = Depends(get_template_manager)):
    """渲染Prompt模板"""
    try:
        # 验证上下文
        is_valid, missing_fields, message = manager.validate_context(request.template_type, request.context)

        if not is_valid:
            # 如果缺少字段，返回详细的错误信息
            return PromptRenderResponse(
                success=False, error=f"上下文验证失败: {message}", validation_warnings=missing_fields
            )

        # 渲染模板
        prompt = manager.render_prompt(
            template_type=request.template_type, style=request.style, context=request.context
        )
...
async def validate_context(
    request: ContextValidationRequest, manager: PromptTemplateManager = Depends(get_template_manager)
):
    """验证上下文是否完整"""
    try:
        is_valid, missing_fields, message = manager.validate_context(request.template_type, request.context)
...
async def get_cultural_keywords(
    category: str = Query("all", description="分类"), manager: PromptTemplateManager = Depends(get_template_manager)
):
    """获取文化关键词"""
    try:
...
async def get_template_example(
    request: TemplateExampleRequest, manager: PromptTemplateManager = Depends(get_template_manager)
):
    """获取模板示例"""
    try:
        example = manager.get_template_example(request.template_type, request.style)
...
async def quick_product_copy(
    request: ProductCopySuggestionRequest, manager: PromptTemplateManager = Depends(get_template_manager)
):
    """快速生成产品文案"""
    try:
        # 构建完整的上下文
        ...
        # 渲染模板
        prompt = manager.render_prompt(template_type="product_copy", style=request.style, context=context)
...
async def quick_slogan(
    request: SloganSuggestionRequest, manager: PromptTemplateManager = Depends(get_template_manager)
):
    """快速生成广告语"""
    try:
        # 构建上下文
        ...
        # 渲染模板
        prompt = manager.render_prompt(template_type="slogan", context=context)
...
async def get_template_details(template_type: str, manager: PromptTemplateManager = Depends(get_template_manager)):
    """获取特定模板的详细信息"""
    try:
        ...
        return {"template_type": template_type, "variants": matching, "count": len(matching)}
...
async def health_check(manager: PromptTemplateManager = Depends(get_template_manager)):
    """健康检查"""
    try:
        templates = manager.list_available_templates()
