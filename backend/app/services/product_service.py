...
from app.schemas.products import ProductCreateRequest, ProductUpdateRequest, ProductStatus
...
class ProductService:
    ...
    def create_product(self, request: ProductCreateRequest, user_id: int) -> Product:
        ...
        if existing:
            ...
            raise BusinessException(code=ErrorCode.RECORD_ALREADY_EXISTS, message=f"产品名称已存在: {request.name}")
        ...
        try:
            ...
            raise BusinessException(code=ErrorCode.DB_INSERT_FAILED, message="产品创建失败")
    ...
    def get_product_by_id(self, product_id: int) -> Product:
        ...
        product = self.db.query(Product).options(joinedload(Product.creator)).filter(Product.id == product_id).first()
        if not product:
    ...
    def get_product_by_name(self, name: str) -> Optional[Product]:
        ...
        """
        # 修复 N+1 查询：预加载 creator 关系
        return self.db.query(Product).options(joinedload(Product.creator)).filter(Product.name == name).first()
    ...
    def list_products(
        ...
        try:
            ...
            query = self.db.query(Product).options(joinedload(Product.creator))  # 预加载创建者信息
            ...
            if region:
                filters.append(
                    or_(Product.origin_province.ilike(f"%{region}%"), Product.origin_city.ilike(f"%{region}%"))
                )
            ...
            raise BusinessException(code=ErrorCode.DB_QUERY_FAILED, message="产品列表查询失败")
    ...
    def get_products_by_category(self, category: str, limit: int = 10) -> List[Product]:
        ...
        try:
            ...
            products = (
                self.db.query(Product)
                .options(joinedload(Product.creator))
                .filter(and_(Product.category == category, Product.status == ModelProductStatus.PUBLISHED))
                .order_by(desc(Product.created_at))
                .limit(limit)
                .all()
            )
    ...
    def update_product(self, product_id: int, request: ProductUpdateRequest, user_id: int) -> Product:
        """更新产品
        ...
        if product.created_by != user_id:
            ...
            if user_role not in ("admin", "super_admin"):
                ...
                raise BusinessException(code=ErrorCode.PERMISSION_DENIED, message="无权更新此产品")
        ...
        try:
            ...
            raise BusinessException(code=ErrorCode.DB_UPDATE_FAILED, message="产品更新失败")
    ...
    def delete_product(self, product_id: int) -> bool:
        ...
        try:
            ...
            raise BusinessException(code=ErrorCode.DB_DELETE_FAILED, message="产品删除失败")
